laydate.render({
  elem: '#test1'
  ,format: 'yyyy年MM月'    /* 表单日期调用 */
});

	$(".submit").on('click',function () {
		var name=$(".billing-address-name").val();
		var card=$(".credit-card-number").val();
		var has=$(".security-code").val();
		var date=$(".expiration-month-and-year").val();
		if(!(name=='')&&!(card=='')&&!(has=='')&&!(date=='')){
					 layer.msg('正在为您生成订单', {
							icon: 16
						,shade: 0.01
							});	
